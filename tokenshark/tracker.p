...
COST_PER_1M = {
    ...
    # 2026-07-11.
    #
    # gpt-4o / gpt-4o-mini / gpt-4-turbo removed: gpt-4o was retired from
    # ChatGPT on 2026-02-13 (OpenAI's own figures put it at 0.1% of daily
    # usage before that), and as of today none of the three appear on
    # OpenAI's current pricing page at all — there's no current official
    # price to fill these placeholders in with. Add them back with a
    # source if you have a specific reason a client still needs them.
    #
    # Prices below are the "Standard" tier, short-context (<=~200K input)
    # rate — what a normal chat.completions.create() call gets by
    # default. NOT handled: long-context pricing (>200K input prices
    # ~2x higher on these models) and the Batch/Flex/Priority tiers,
    # which price differently and aren't inspectable from the request
    # kwargs the same way `stream` is. A large-context or non-Standard
    # call will be mispriced by this table. Flagging as a known gap, not
    # fixing now — out of today's scope.
    "gpt-5.6-sol": {
        "in": 5.00, "out": 30.00, "cache_create": 6.25, "cache_read": 0.50,
    },
    "gpt-5.6-terra": {
        "in": 2.50, "out": 15.00, "cache_create": 3.125, "cache_read": 0.25,
    },
    "gpt-5.6-luna": {
        "in": 1.00, "out": 6.00, "cache_create": 1.25, "cache_read": 0.10,
    },
    "gpt-5.5": {"in": 5.00, "out": 30.00, "cache_read": 0.50},  # no cache-write line on the pricing page for this model
    "gpt-5.4": {"in": 2.50, "out": 15.00, "cache_read": 0.25},  # no cache-write line on the pricing page for this model
    "gpt-5.4-mini": {"in": 0.75, "out": 4.50, "cache_read": 0.075},
    "gpt-5.4-nano": {"in": 0.20, "out": 1.25, "cache_read": 0.02},

    # --- Anthropic --- verified against claude.com/pricing, 2026-07-11.
    #
    # BUG FIXED: cache_create below used to store the *0.25x premium* on
    # top of base input (e.g. 1.25 = 0.25 x Opus's $5.00 base) instead of
    # the *full 1.25x rate Anthropic actually bills for every cache-write
    # token* (6.25 = 1.25 x $5.00). That under-reported Anthropic
    # cache-write cost by ~5x for every model in this table. cache_read
    # (0.10x base) was already correct and is unchanged.
    #
    # Model IDs verified against platform.claude.com/docs (Model IDs and
    # versioning + Models overview). Claude 4.6-generation-and-later IDs
    # are dateless pinned snapshots (claude-sonnet-4-6 is not an alias,
    # it *is* the model). Pre-4.6 models keep a dated snapshot ID plus a
    # dateless alias that resolves to it — both are included below since
    # the API may echo back either form depending on which was requested.
    # Haiku 4.5 has NO dateless form: "claude-haiku-4-5" alone is not a
    # valid model ID, only "claude-haiku-4-5-20251001" is — the old key
    # here would never have matched a real response.
    #
    # Omitted: claude-opus-4-1. Four generations behind current Opus,
    # pricing-page-listed but I couldn't confirm its exact model ID
    # string here (no network access to check) and it's the least likely
    # of the legacy tier to still be "used a lot" — add it back with a
    # confirmed ID if you need it.
    "claude-fable-5": {
        "in": 10.00, "out": 50.00, "cache_create": 12.50, "cache_read": 1.00,
    },
    "claude-opus-4-8": {
        "in": 5.00, "out": 25.00, "cache_create": 6.25, "cache_read": 0.50,
    },
    "claude-opus-4-7": {
        "in": 5.00, "out": 25.00, "cache_create": 6.25, "cache_read": 0.50,
    },
    ...
    # Introductory pricing, valid through 2026-08-31 -- becomes
    # 3.00 / 15.00 / 3.75 / 0.30 on 2026-09-01. Update this entry then.
    "claude-sonnet-5": {
        "in": 2.00, "out": 10.00, "cache_create": 2.50, "cache_read": 0.20,
    },
    "claude-sonnet-4-6": {
        "in": 3.00, "out": 15.00, "cache_create": 3.75, "cache_read": 0.30,
    },
    "claude-sonnet-4-5": {
        "in": 3.00, "out": 15.00, "cache_create": 3.75, "cache_read": 0.30,
    },
    ...
    "claude-haiku-4-5-20251001": {
        "in": 1.00, "out": 5.00, "cache_create": 1.25, "cache_read": 0.10,
    },
